...
- Vídeo (`--video`) só para interação que precisa de prova em movimento: preenchimento de formulário, clique de botão que muda estado, e também entrar na listagem e clicar na paginação (Abner confirmou que esse vídeo faz sentido). Não gravar página onde a funcionalidade nem aparece (ex.: listagem sem paginação, como a tentativa com filtro Toyota na PR #516) nem tentativas exploratórias. Confirmar o elemento sem vídeo antes. Status, canonical e visual estático ficam em print/HTTP. Pedido em 2026-09-25.
